...
public class SerialGamePersistenceManager implements GamePersistenceManager{
    ...
    public void updateGame(GameDTO game) {
        String fileName = saveDirectory + File.separator + "game_" + game.id() + ".ser";

        try (FileOutputStream fileOut = new FileOutputStream(fileName);
             ObjectOutputStream out = new ObjectOutputStream(fileOut)) {

            out.writeObject(game);
            System.out.println("Game " + game.id() + " saved correctly.");

        } catch (IOException e) {
            System.err.println("Error: game " + game.id() + " was not saved correctly: " + e.getMessage());
            e.printStackTrace();
        }
    }
