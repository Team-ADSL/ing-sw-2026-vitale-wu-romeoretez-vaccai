...
public class EndGameTest {
    ...
    void getId_returnsCorrectId() {
        EndGame building = new EndGame("eg_01", 5, 3, 1, Optional.empty(), BuildingEffect.END_PP_BONUS, null);
        assertEquals("eg_01", building.getId());
    }

    @Test
    void endBuilderMultiplier_endGameTrigger_setsBuilderMultiplierTo2() {
        EndGame building = new EndGame("eg", 5, 3, 1, Optional.empty(), BuildingEffect.END_BUILDER_MULTIPLIER, null);
    ...
    void ppCompleteSet_endGameTrigger_awardsMinCardCount_times6() {
        for (CardType type : new CardType[]{CardType.HUNTER, CardType.GATHERER, CardType.SHAMAN,
            ...
            player.getCards().get(type).add(new Hunter("h1", false, 1, Optional.empty()));
            player.getCards().get(type).add(new Hunter("h2", false, 1, Optional.empty()));
        }
        EndGame building = new EndGame("eg", 5, 3, 1, Optional.empty(), BuildingEffect.PP_COMPLETE_SET, null);
        building.activeEffect(Set.of(player), Trigger.END_GAME);
    ...
    void endCharacterMultiplier_endGameTrigger_awardsPPBasedOnCardTypeCount() {
        ...
        EndGame building = new EndGame("eg", 5, 3, 1, Optional.empty(), BuildingEffect.END_CHARACTER_MULTIPLIER, CardType.HUNTER);
        building.activeEffect(Set.of(player), Trigger.END_GAME);
    ...
    void endCharacterMultiplier_noCardsOfType_awardsZero() {
        EndGame building = new EndGame("eg", 5, 3, 1, Optional.empty(), BuildingEffect.END_CHARACTER_MULTIPLIER, CardType.INVENTOR);
        building.activeEffect(Set.of(player), Trigger.END_GAME);
        assertEquals(0, player.getPp());
    }
    ...
    void endPpBonus_endGameTrigger_awards25PP() {
        EndGame building = new EndGame("eg", 5, 3, 1, Optional.empty(), BuildingEffect.END_PP_BONUS, null);
        building.activeEffect(Set.of(player), Trigger.END_GAME);
