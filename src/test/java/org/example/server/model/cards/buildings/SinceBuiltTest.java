...
public class SinceBuiltTest {
    ...
    void getId_returnsCorrectId() {
        SinceBuilt building = new SinceBuilt("since_built_01", 5, 3, Trigger.DRAWING, 1, Optional.empty(), BuildingEffect.FOOD_COMPLETE_SET);
        assertEquals("since_built_01", building.getId());
    }

    @Test
    void foodCompleteSet_lessThan6Types_noFoodAwarded() {
        SinceBuilt building = new SinceBuilt("sb", 5, 3, Trigger.DRAWING, 1, Optional.empty(), BuildingEffect.FOOD_COMPLETE_SET);

        player.setLastPick(new Hunter("h", false, 1, Optional.empty()));
        building.activeEffect(Set.of(player), Trigger.DRAWING);

        player.setLastPick(new Gatherer("g", 0, 1, Optional.empty()));
        building.activeEffect(Set.of(player), Trigger.DRAWING);

        player.setLastPick(new Shaman("s", 1, 1, Optional.empty()));
        building.activeEffect(Set.of(player), Trigger.DRAWING);

        player.setLastPick(new Builder("b", 0, 0, 1, Optional.empty()));
        building.activeEffect(Set.of(player), Trigger.DRAWING);

        player.setLastPick(new Inventor("i", Icon.BOAT, 1, Optional.empty()));
        building.activeEffect(Set.of(player), Trigger.DRAWING);
    ...
    void foodCompleteSet_allSixTypes_awards5Food() {
        ...
        player.setLastPick(new Artist("a", 1, Optional.empty()));
        building.activeEffect(Set.of(player), Trigger.DRAWING);
    ...
    void coupleInventor_oneInventor_noFoodAwarded() {
        SinceBuilt building = new SinceBuilt("sb", 5, 3, Trigger.DRAWING, 1, Optional.empty(), BuildingEffect.COUPLE_INVENTOR);
        player.setLastPick(new Inventor("i", Icon.BOAT, 1, Optional.empty()));
        building.activeEffect(Set.of(player), Trigger.DRAWING);
        assertEquals(0, player.getFood());
    }
    ...
    void coupleInventor_twoInventors_awards3Food() {
        ...
        player.setLastPick(new Inventor("i1", Icon.BOAT, 1, Optional.empty()));
        building.activeEffect(Set.of(player), Trigger.DRAWING);

        player.setLastPick(new Inventor("i2", Icon.ROPE, 1, Optional.empty()));
        building.activeEffect(Set.of(player), Trigger.DRAWING);
